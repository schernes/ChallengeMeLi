...
def insertHistorico(x):
    miCursor.executemany('''INSERT INTO PUBLIC_FILES_HISTORY
    VALUES(NULL,?,?)''', x)
...
def buscaDB(x):
    miCursor.execute('''SELECT FILE_ID,
    LAST_CHANGE_DATE
    FROM FILES
    WHERE FILE_ID = ?''', [x])
...
def borrarArchivosDB(x):
    ...
    miCursor.executemany('''INSERT INTO TABLA_PUENTE
    VALUES(NULL,?,?)''', x)
